...
export class AuthController {
  ...
  handleRedirect(@Req() request: Request,@Res() response: Response) {
    ...
    response.redirect(
      `https://wangpa-hlrjfv3lh-malao-ang.vercel.app/homepage/?email=${request.user['profile']['emails'][0]['value']}&name=${request.user['profile']['displayName']}&picture=${request.user['profile']['photos'][0]['value']}&token=${request.user['accessToken']}` ,
    );
  }
